...
class HBOS(BaseDetector):
    ...
    def __init__(self, n_bins=10, alpha=0.1, tol=0.5, contamination=0.1):
        super(HBOS, self).__init__(contamination=min(0.499, contamination))
        self.n_bins = n_bins
        self.alpha = alpha
        self.tol = tol
    ...
    def fit(self, X, y=None):
        ...
        self.dimension_contribution = self.decision_scores_per_var.copy()
        self._process_decision_scores()
        self._process_dimension_contribution()
        return self

    def _process_dimension_contribution(self):
        assert self.dimension_contribution is not None, "Dimension contribution should not be empty. Please check the input data and the number of bins to make sure the histogram is properly constructed."
        self.dimension_contribution  = self.dimension_contribution - self.dimension_contribution.min(axis=1)[:, np.newaxis] + 0.01
        self.dimension_contribution = self.dimension_contribution / self.dimension_contribution.sum(axis=1)[:, np.newaxis]
...
def _calculate_outlier_scores_auto(X, bin_edges, hist, alpha,
    ...
    for i in range(n_features):
        ...
        # for index, t in enumerate(out_score_i):
        #     if t > 0:
        #         print(f'Warning: The outlier score for bin {index} on dimension {i} is positive ({t}). This may indicate that the histogram density value for this bin is greater than 1, which could be due to an issue with the input data or the number of bins. Please check the input data and the number of bins to make sure the histogram is properly constructed.')
...
def _calculate_outlier_scores(X, bin_edges, hist, n_bins, alpha,
    ...
    for i in range(n_features):
        ...
        # assert all(out_score_i <= 0), "The outlier scores should be non-positive since they are log2 of density values. Please check the input data and the number of bins to make sure the histogram is properly constructed."
        for j in range(n_samples):
