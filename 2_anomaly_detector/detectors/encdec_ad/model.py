...
class EncDecAD(nn.Module):
    ...
    def anomaly_detection(self, ts: np.ndarray) -> Tuple[np.ndarray,np.ndarray]:
        self.eval()
        ...
        anomaly_scores_per_var = self.anomaly_scorer.forward(errors).detach().numpy()
        return self.anomaly_scorer.forward(errors.mean(dim=1)).detach().numpy(), anomaly_scores_per_var
